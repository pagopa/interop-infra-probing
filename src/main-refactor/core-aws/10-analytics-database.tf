...
locals {
  ...
  timestream_organization  = local.timestream_instance_name
  bucket_prefix_name       = "probing-telemetry"
}

data "aws_secretsmanager_random_password" "probing_analytics_admin" {
  password_length            = 16
...
resource "aws_secretsmanager_secret_version" "probing_analytics_admin" {
  ...
  secret_string = jsonencode({
    ...
    username                = "admin"
    password                = data.aws_secretsmanager_random_password.probing_analytics_admin.random_password
...
resource "aws_timestreaminfluxdb_db_instance" "probing_analytics" {
  ...
  allocated_storage = var.probing_analytics_instance_allocated_storage
  db_instance_type  = var.probing_analytics_instance_type
...
resource "aws_cloudwatch_metric_alarm" "probing_analytics_instance_disk_utilization" {
  alarm_name        = format("%s-influxdb-instance-disk-utilization-%s", aws_timestreaminfluxdb_db_instance.probing_analytics.name, var.env)
  alarm_description = format("Disk utilization alarm for the %s InfluxDB instance", aws_timestreaminfluxdb_db_instance.probing_analytics.name)
  ...
  evaluation_periods  = var.probing_analytics_instance_disk_utilization_evaluation_periods
  datapoints_to_alarm = var.probing_analytics_instance_disk_utilization_alarm_datapoints
}
...
# Since the command uses the 'influx create config' to create a local configuration file, be sure to have no local configuration with the same name. See more: https://docs.influxdata.com/influxdb/v2/reference/cli/influx/config/create/
resource "terraform_data" "probing_analytics_store_admin_token" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
      ...
      ADMIN_TOKEN=$(influx auth create --user "$ADMIN_USERNAME" --description "Created with Terraform on $DATE" --operator --json | jq -r '.token')
